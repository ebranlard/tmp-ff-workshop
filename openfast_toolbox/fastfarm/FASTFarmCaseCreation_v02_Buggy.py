...
class FFCaseCreation:
    ...
    def _determine_resolutions_from_dummy_amrwind_grid(self):
        ...
        print_bold(f'       High-resolution: ds: {amr.ds_high_les} m, dt: {amr.dt_high_les} s')
        print_bold(f'       Low-resolution:  ds: {amr.ds_low_les} m, dt: {amr.dt_low_les} s')
    ...
    def setTemplateFilename(self, templatePath=None, templateFiles=None, templateFSTF=None):
        ...
        # Join template path if necessary
        if templatePath is not None:
            ...
            for key, value in templateFiles.items():
                if value == 'unused' or value is None:
                    continue
                templateFiles[key] = os.path.join(self.templatePath, f"{value}")
        ...
        if templateFSTF is not None:
            ...
            # --- 
            fstfFilename = templateFiles['FFfilename']
            baseDir = os.path.dirname(fstfFilename)
            fstf = FASTInputFile(fstfFilename)
            ...
            dck = FASTInputDeck(fstFilename, verbose=True)
            ...
            for key_deck, key_tpl in KEY_MAP.items():
                if key_deck in fread.keys(): 
                    if key_tpl not in templateFiles:
                        filebase = fread[key_deck]
                        if '.T.' in filebase:
                            filebase = fread[key_deck].rsplit('.T.', 1)[0]+'.T'
                        templateFiles[key_tpl] = filebase
                        INFO(f'Using template file {key_tpl}={filebase}')
        ...
        # TODO TODO TODO Replace this by a dictionary instead of all the individuals "filepath"
        for key, value in templateFiles.items():
            if value == 'unused' or value is None:
                continue
            ...
            if key == 'EDfilename':
            ...
            elif key == 'controllerInputfilename' or key =='controllerInputfilepath':
                ...
                self.FFfilename = os.path.basename(value)
